...
const Bank = () => {
    ...
    */
    ...
    return (
        <>
            <Flex width="100%" className="bank-container">
                {isConnected ? (
                    <Flex direction="column" width="100%">
                        <Flex alignItems="center" mb="1rem" className="bank-container-flex">
                            <Heading as='h2' size='xl' className="bank-container-heading">
                                Your balance in the Bank
                            </Heading>
                            {userBalanceLoading ? (
                                <Spinner />
                            ) : (
                                <Text mt="1rem" className="bank-container-text">{formatEther(userBalance?.toString())} Eth</Text>
                            )}
                        </Flex>

                        <Flex alignItems="center" mb="1rem" className="bank-container-flex">
                            <Heading as='h2' size='xl' mt="2rem" className="bank-container-heading">
                                Deposit
                            </Heading>
                            <Flex mt="1rem" className="bank-container-flex-flex">
                        ...
                        <Flex alignItems="center" mb="1rem" className="bank-container-flex">
                            ...
                            {parseFloat(debouncedWithdrawAmount) > parseFloat(formatEther(userBalance)) && <Text color='red' mt='.5rem' className="bank-container-text">You cannot withdraw more than {parseFloat(formatEther(userBalance))} ETH.</Text>}
                        </Flex>
                        <Flex alignItems="center" mb="1rem" className="bank-container-flex">
                            <Heading as='h2' size='xl' mt="2rem" className="bank-container-heading">
                                Deposit Events
                            </Heading>
                            <Flex mt="1rem" direction="column" className="bank-container-flex-flex">
                            {depositEvents.length > 0 ? depositEvents.map((event) => {
                                return <Flex key={crypto.randomUUID()}><Text className="bank-container-text">
                                {event.address} - {formatEther(event.amount)} Eth</Text>
                                </Flex>
                            }) : (
                                <Text className="bank-container-text">No Deposit Events</Text>
                            )}
                            </Flex>
                        </Flex>
                        <Flex alignItems="center" mb="1rem" className="bank-container-flex">
                            ...
                            {widthdrawEvent.length > 0 ? widthdrawEvent.map((event) => {
                                ...
                                <Text className="bank-container-text">No Withdraw Events</Text>
                            )}   
                            </Flex>
                        </Flex>
                    </Flex>
                    ...
                    <Flex p="2rem" justifyContent="center" alignItems="center" width="100%">
                        <Text className="bank-container-text">Please connect your Wallet</Text>
                    </Flex>
                )}
